...
public class ProjectTest {
	...
	public void playerWithDirectionDownMovesDownUnlessAtBottom() {
		...
		//spawnHarrierGamePieces()
	@Test
	public void weCannotWriteTestsForThisFunctionBecauseItUsesRandomNumbersToSpawn() {
		Model m = new Model(1,1,1,1);
		m.spawnHarrierGamePieces();
		assert true;
	}
		//spawnOspreyGamePieces()
	@Test
	public void weAlsoCannotWriteTestsForThisFunctionBecauseItUsesRandomNumbersToSpawn() {
		Model m = new Model(1,1,1,1);
		m.spawnOspreyGamePieces();
		assert true;
	}
		//clearCurrentGP()
	@Test
	...
	public void numOfQuestionsIsNegativeOneIfNoQuestions() {
		...
		//resetModel()
	@Test
	public void resettingModelSetsRoundTo0() {
		Model m = new Model(1,1,1,1);
		m.setRound(1);
		m.generateOspreyQuestions2();
		m.resetModel();
		assert m.getRound() == 0;
	}
	@Test
	public void resettingModelSetsTotalLevelTicksTo0() {
		Model m = new Model(1,1,1,1);
		m.setTotalLevelTicks(1);
		m.generateOspreyQuestions();
		m.resetModel();
		assert m.getTotalLevelTicks() == 0;
	}
		//resetQuestionNum()
	@Test
	public void resettingQuestionNumSetsQuestionNumTo0() {
		Model m = new Model(1,1,1,1);
		m.incrQuestionNum();
		m.resetQuestionNum();
		assert m.getQuestionNum() == 0;
	}
		//quizOver()
	@Test
	public void quizIsOverIfQuestionNumGreaterThanNumQuestions() {
		Model m = new Model(1,1,1,1);
		m.resetQuestionNum();
		m.incrQuestionNum();
		m.setNumberOfQuestions(0);
		assert m.quizOver();
	}
	@Test
	public void quizIsNotOverIfQuestionNumLessThanNumQuestions() {
		Model m = new Model(1,1,1,1);
		m.resetQuestionNum();
		m.incrQuestionNum();
		m.setNumberOfQuestions(2);
		assert !m.quizOver();
	}
		//lastQuestion()
	@Test
	public void lastQuestionIsTrueIfQuestionNumEqualsNumOfQuestions() {
		Model m = new Model(1,1,1,1);
		m.resetQuestionNum();
		m.setNumberOfQuestions(0);
		assert m.lastQuestion();
	}
	@Test
	public void lastQuestionIsFalseIfQuestionNumGreaterThanNumOfQuestions() {
		Model m = new Model(1,1,1,1);
		m.resetQuestionNum();
		m.incrQuestionNum();
		m.setNumberOfQuestions(0);
		...
		//setCorrectAnswer() and getCorrectAnswer()
	@Test
	public void correctAnswerSetIsCorrectAnswer() {
		Model m = new Model(1,1,1,1);
		m.setCorrectAnswer("The answer");
		assert m.getCorrectAnswer().equals("The answer");
	}
		//hasMoreFacts()
	@Test
	public void havingMoreFactsMakesHasMoreFactsTrue() {
		Model m = new Model(1,1,1,1);
		m.generateHarrierQuestions();
		for (int i = 0 ; i < 15 ; i++) {
			m.incrFactIndex();
		}
		assert !m.hasMoreFacts();
	}
		//getCurrentFact()
	@Test
	public void currentFactForHarrierWithCFIAt0IsAboutRodents() {
		Model m = new Model(1,1,1,1);
		m.generateHarrierQuestions();
		assert m.getCurrentFact().equals("Northern Harriers, eat rodents");
	}
		//setSpecialFoodEaten() and getSpecialFoodEaten()
	@Test
	public void ifSpecialFoodIsSetEatenThenItIsEaten() {
		Model m = new Model(1,1,1,1);
		m.setSpecialFoodEaten(true);
		assert m.specialFoodEaten();
	}
	@Test
	public void ifSpecialFoodIsSetNotEatenThenItIsNotEaten() {
		Model m = new Model(1,1,1,1);
		m.setSpecialFoodEaten(false);
		assert !m.specialFoodEaten();
	}
		//getAvailableFacts()
	@Test
	public void availableFactsForHarrierAreNotNullAtStart() {
		Model m = new Model(1,1,1,1);
		assert m.getAvaliableFacts() != null;
	}
	...
	//Tests for Controller
		//Cannot write JUnit tests for controller due to userinput required and use of view.
	
	//Tests for View
		//Cannot write JUnit tests for view due to graphical nature of file.
}	
